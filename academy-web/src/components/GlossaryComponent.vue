  ...
  <div class="glossary-container">
    ...
    <TransitionGroup name="list" tag="div" class="terms-grid" v-if="filteredTerms.length > 0">
      ...
      >
        ...
        <!-- Интерактивная зона визуализации (ASCII или Код) -->
        <div class="visual-section" v-if="expandedCards[term.id]">
          <div class="visual-header">
            <span class="visual-label">{{ term.visualType === 'code' ? '💻 Java / GLSL Код' : '📊 ASCII Схема' }}</span>
            <button @click="copyVisualText(term.visualContent)" class="copy-btn" title="Копировать в буфер">
              📋
            </button>
          </div>
          <pre class="visual-content"><code>{{ term.visualContent }}</code></pre>
        </div>
        ...
        <div class="card-actions">
          ...
          >
            {{ expandedCards[term.id] ? '▲ Скрыть визуализацию' : '▼ Показать ASCII-схему / Код' }}
          </button>
        </div>
      </div>
    </TransitionGroup>
