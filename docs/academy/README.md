...
Учебник разбит на 10 специализированных глав. Вот что в них рассматривается:
...
1. **Глава 1 (Память)**: Если мы не научим JVM не мусорить в память (Zero-Allocation), то любые графические конвейеры в последующих главах будут заикаться из-за GC-пауз (пауз Сборщика Мусора). Сначала настраиваем "мозг" и память на CPU.
2. **Глава 2 (OpenGL)**: Память готова. Мы хотим вывести картинку. Мы переносим данные на GPU и учим его рисовать. Но классический подход заставляет процессор делать тысячи звонков (Draw Calls), упираясь в PCIe шину (высокоскоростную шину передачи данных) (CPU Bottleneck).
...
Начинай с **[Главы 1](chapter_1_jvm_and_memory.md)** — там закладывается фундамент всей архитектуры.
