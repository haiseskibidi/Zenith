...
- **StepUpHandler.java**: Модульный обработчик автоматического подъема сущностей по ступеням (STEP) и наклонным пандусам (SMOOTH). Рассчитывает временный AABB подъем, коллизии и возвращает результирующее горизонтальное и вертикальное смещение.
