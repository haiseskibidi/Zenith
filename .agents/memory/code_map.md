...
- **ChunkRenderSystem.java**: Система отрисовки мира. (v2.8 UPDATED) Реализует спаренный MDI-рендеринг, 100% Zero-Allocation 3D BFS Occlusion Culling с правилом строгой монотонности (Strict Monotonicity), фронтальную сортировку непрозрачных чанков (Front-to-Back) для активации Early-Z на iGPU, и умное планирование ремеша чанков.
- **EntityRenderSystem.java**: Отрисовка динамических сущностей (игроки, мобы) и вьюмоделей рук/предметов.
- **ItemEntityStacker.java**: (NEW) Рендеринг визуального стаканья блоков (диагональная фиксированная стопка из 1-3 моделей, как в Minecraft) и предметов (каскадный сдвиг лесенкой до 5 моделей по локальным координатам) с плавным вращением по оси Y.
...
### com.za.zenith.world.generation.caves (v4.5 UPDATED)
