...
- **RenderPipeline.java**: Главный оркестратор графического конвейера. Управляет жизненным циклом кадра, переключением буферов и делегирует отрисовку специализированным системам. Вычисляет циклы лунных фаз, `isNight` состояние и направляет серебряные лучи Moonlight Shafts сквозь листву ночью.
...
- **ChunkRenderSystem.java**: Система отрисовки мира. (UPDATED) Реализует спаренный MDI-рендеринг (`opaqueBatches[2]`, `translucentBatches[2]`), кэширование матрицы фрустума `lastFrustumMatrix`, двухэтапный иерархический фрустум-куллинг (AABB чанка-столбца + AABB секции) и 100% Zero-Allocation.
