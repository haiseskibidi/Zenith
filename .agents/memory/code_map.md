...
- **EntityRenderSystem.java**: Отрисовка динамических сущностей (игроки, мобы) и вьюмоделей рук/предметов. Оптимизирована для Zero-Allocation: проверки видимости фрустума переведены на примитивные координаты AABB с использованием пула векторов, исключая скрытые аллокации JOML `Vector3f` на кадр.
...
Функции: Добавлена Zero-Alloc мапа компонентов (`components`), методы `getComponent()`, `addComponent()` и `hasComponent()`. Реализует интерполяцию (включая Zero-Allocation перегрузки `getInterpolatedPosition`/`getInterpolatedRotation`, принимающие destination-вектор для предотвращения heap allocations), систему выталкивания из блоков (`Unstuck`) и общую логику перемещения `move()` с коллизиями.
