...
- **EntityRenderSystem.java**: Оркестрирует отрисовку динамических сущностей (игроки, мобы) и делегирует рендеринг вьюмодели рук первого лица. Оптимизирована для Zero-Allocation: проверки видимости фрустума переведены на примитивные координаты AABB.
- **ViewmodelRenderPass.java (NEW)**: Инкапсулирует настройку проекции рук, применение аффинных трансформаций освещения камеры и вызов `ViewmodelRenderer.render` для рук первого лица.
- **EntityRenderHelper.java (NEW)**: Группирует вспомогательные методы отрисовки конкретных типов сущностей (скауты, предметы, декорации, remote players), разгружая основную систему рендеринга.
- **ItemEntityStacker.java**: Рендеринг визуального стаканья блоков (диагональная фиксированная стопка из 1-3 моделей, как в Minecraft) и предметов (каскадный сдвиг лесенкой до 5 моделей по локальным координатам) с плавным вращением по оси Y. Оптимизирован под Zero-Allocation расчет вращения через пулы векторов.
...
Назначение: Фасадный класс управления состоянием мира, координирующий специализированные подсистемы.
Функции: 
- **Subsystem Orchestration**: Делегирует задачи асинхронного конвейера чанков, симуляции погоды/облаков, повреждений блоков и сущностей выделенным менеджерам.
- **API Compatibility**: Предоставляет методы-делегаты для обеспечения полной обратной совместимости систем рендеринга и ввода.
- **Zero-Allocation getBlock**: Оптимизированный доступ к блокам с быстрым обходом хэш-карты повреждений при ее пустоте.
- **L1 Chunk Cache**: Ускоренный доступ к данным чанков с использованием `ThreadLocal` кэша.
...
Функции: Добавлена Zero-Alloc мапа компонентов (`components`), методы `getComponent()`, `addComponent()` и `hasComponent()`. Реализует интерполяцию (включая Zero-Allocation перегрузки `getInterpolatedPosition`/`getInterpolatedRotation`, принимающие destination-вектор для предотвращения heap allocations) и делегирует логику перемещения `move()` в `EntityPhysicsProcessor`.
- **com.za.zenith.world.physics.EntityPhysicsProcessor (NEW)**: Реализует высокопроизводительное перемещение `move()`, обработку вертикальных и горизонтальных коллизий, а также проверку застреваний `isCollidingAt()`.
- **com.za.zenith.world.physics.EntityRampPhysicsHelper (NEW)**: Вспомогательный класс для обработки движения по наклонным поверхностям (рампам), детекции пропусков коллизий над головой и плавного прилипания/подъёма по склонам.
...
### com.za.zenith.world
- **World.java (UPDATED)**: Фасад управления состоянием мира.
- **WorldDamageManager.java (NEW)**: Менеджер повреждений блоков. Хранит `blockDamageMap` с объектами `BlockDamageInstance` (урон + история шрамов в `Vector4f`). Реализует логику задержки регенерации (5 сек) и плавного затухания шрамов (изменение компоненты `w`).
- **WorldEntityManager.java (NEW)**: Менеджер сущностей и пространственного индекса предметов `groundEntityMap` в чанках. Реализует логику притяжения/подбора предметов и куллинг далёких ресурсов.

### com.za.zenith.world.blocks
- **BlockDefinition.java (UPDATED)**: Контейнер свойств вокселя. Снижен размер файла до 558 строк. Поддерживает битовые маски метаданных и Data Components.
- **BlockGravitySystem.java (NEW)**: Система гравитации для сыпучих блоков. Отвечает за логику `checkFall` и скрытие блоков на GPU при падении.
- **BlockDropManager.java (NEW)**: Менеджер выпадения ресурсов при разрушении блоков по `DropRules` и legacy-правилам.
- **BlockInteractionHandler.java (NEW)**: Обработчик кликов ЛКМ/ПКМ на блоках на основе модульных компонентов и интерактивных зон.

### com.za.zenith.world.chunks
- **WorldChunkManager.java (NEW)**: Фоновый менеджер чанков. Управляет очередью загрузки чанков, асинхронными пулами `chunkGenExecutor`/`lightExecutor` и логикой Lazy Unload выгрузки чанков вокруг игрока.

### com.za.zenith.world.physics
- **FluidFlowCalculator.java (NEW)**: Изолированный статический калькулятор физики жидкостей. Вычисляет интерполированные вектора направления течения жидкостей с поддержкой перегрузок Zero-Allocation векторов.
...
- **StepUpHandler.java**: Модульный обработчик автоматического подъема сущностей по ступеням (STEP) и наклонным пандусам (SMOOTH). Рассчитывает временный AABB подъем, коллизии и возвращает результирующее смещение. Содержит логику проверки коллизий с потолком (ceiling check) для предотвращения телепортации и прохождения сквозь низкие перекрытия (например, деревянные опоры шахт).
...
- **CloudSimulationSystem.java (NEW)**: Система симуляции движения облаков с учетом направления ветра и интенсивности дождя. Управляет плавным проявлением и затуханием объектов `CloudInstance`.
...
- **InputManager.java (UPDATED)**: Event Dispatcher. Хранит глобальный контекст, централизованно отслеживает фазы нажатий на основе плоских Zero-Alloc буферов и делегирует события специализированным хэндлерам. Сокращен до 392 строк.
...
- **handlers/InventoryClickHandler.java (NEW)**: Обработчик кликов инвентаря, драг-распределения и взаимодействия с панелью разработчика.
...
### com.za.zenith.world.chunks
- **ChunkMeshGenerator.java**: Генератор геометрии. Внедрена поддержка `FLAG_TRANSLUCENT` и погранного окрашивания (`isFaceTinted`). Устранены критические NPE при обращении к соседям. Оптимизирован под Zero-Allocation: использует временный буфер `tempFaceVertices` в `MeshData` для вычисления координат вершин на лету без создания массивов `float[][]`, а culling-проверки переведены на примитивные флоат-координаты AABB для предотвращения скрытых аллокаций `Vector3f` через `.getMin()`/`.getMax()`.
