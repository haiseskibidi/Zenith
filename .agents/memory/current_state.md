...
### 🚀 Архитектура, Обучение и Движок (Zenith v2.6)
- **Low-Level & Hardware Architecture Course (Глубокий Академический Курс)**:
  - Разработан и полностью интегрирован новый глубокий трек по низкоуровневой архитектуре ПК из 5 обширных глав (объемом в среднем 500+ строк на главу), обеспечивающий бесшовный логический переход от физики полупроводников к операционным системам.
  - **Глава 0. Кремний, физика транзисторов и сумматоры**: Охватывает метод Чохральского, EUV-фотолитографию ASML, зонную теорию проводимости, физику p-n перехода и MOSFET затворов (High-k диэлектрики $HfO_2$ vs $SiO_2$), КМОП-логику (NOT, NAND, NOR, XOR с формулой динамической мощности $P \approx C V^2 f$), Half/Full Adders, и детальное сравнение Ripple-Carry vs Carry Lookahead Adders.
  - **Глава 1. Анатомия CPU**: Разбирает тактовую физику (кварц, PLL), Гарвардский раздельный L1 кэш (L1i/L1d), устройство ядра (GPR, RIP, RFLAGS, RSP/RBP аппаратный стек), 5-стадийный цикл инструкций с разбором `ADD RAX, [RBP - 8]`, конвейерные конфликты (RAW, WAR, WAW), обход данных (forwarding), 2-битный насыщающий Smith-предсказатель переходов, внеочередное выполнение (OoO Tomasulo, ROB, RAT), CISC-в-RISC декодирование и SIMD (SSE/AVX-512) с рабочим Java Vector API примером и встроенным бенчмарком.
  - **Глава 2. Иерархия памяти**: Покрывает SRAM (6T-триггеры) vs DRAM (1T1C утечки, precharge, регенерация $tRFC$/$tREFI$), пирамиду задержек с аналогией шеф-повара, структуру кэш-линий (Tag, Index, Offset) для Set-Associative кэша, MESI-когерентность, Spatial/Temporal Locality, Pointer Chasing в JVM и решение Zenith на плоских Flat-массивах примитивов и выравнивании `@Contended` (False Sharing).
  - **Глава 3. Шоссе PCIe и DMA**: Описывает топологию Root Complex и PCH чипсета по DMI, дифференциальное кодирование (CMRR), скремблирование, NRZ vs PAM4 (сравнение глазковых диаграмм), Scatter-Gather DMA, Pinned Memory, Resizable BAR, DirectStorage, и решения Zenith (Mesh Pool + MultiDraw Indirect MDI с примером GLSL-шейдера на базе `gl_DrawID`).
  - **Глава 4. Операционная система**: Раскрывает изоляцию процессов (виртуальная память, PML4 таблица страниц, MMU/TLB, Stack vs Heap), Page Faults (Minor vs Major), свопинг и трэшинг (Thrashing), аппаратную логику Context Switch (прерывания, сброс конвейера, TCB, смена CR3 и инвалидация TLB), и преимущества Lock-Free CAS-очередей в ThreadPool Zenith.
