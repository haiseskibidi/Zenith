...
### 🚀 Архитектура, Обучение и Движок (Zenith v2.8)
- **Realistic Atmosphere & Monolith Sun Glare (Atmosphere v2.1)**:
  - **Soft-Halo Sun Bloom (Баг резкости в центре исправлен)**: Устранено схлопывание шага радиального размытия в ноль при прямом взгляде на солнце. В шейдере `sun_shafts.glsl` шаг сэмплинга динамически перераспределяется с помощью коэффициента `glareShift = pow(lookAtSun, 12.0)`. При прямом взгляде минимальный радиус размытия принудительно устанавливается в 12% экрана (`0.12`). Это создает роскошный объемный Bloom-ореол рассеянного света, полностью убирая жесткие Toon-грани солнца, сохраняя плавный классический радиальный блюр в стороны.
  - **Crepuscular Twilight Rays (Закатные лучи)**: Реализованы длинные атмосферные сумеречные лучи заката. Введена кусочно-гладкая функция высоты `sunHeight = sunDir.y`. Когда солнце опускается за горизонт (до `sunHeight = -0.25`), лучи сочно и мягко пробиваются снизу экрана вверх, плавно угасая до полной темноты ночью, что полностью исключает лучи из-под земли в ночное время.
  - **Smooth Screen-Space Transitions ("Sun Lives Its Own Life")**: Границы видимости солнца на экране расширены до `[-0.6f, 1.6f]`. Это позволяет лучам плавно и бесшовно скользить по экрану, исключая любые резкие исчезновения при повороте камеры и заходе солнца за горизонт.
  - **Zenith Height-Based Attenuation**: Днем при подъеме солнца к зениту интенсивность лучей плавно и нелинейно гасится по формуле `(1.0f - sunHeight)^1.3f`, предотвращая ослепление и выжигание неба в белый цвет в полдень, при этом сохраняя мягкий ослепляющий эффект (glare) при прямом взгляде вверх.
  - **Dust Motes Elimination**: Парящие "кругляшки" (ambient particles) полностью отключены в `RenderPipeline` для соответствия строгому реалистичному стилю, разгрузив CPU и GPU от тиков и отрисовки 300 билбордов.
- **Discrete Block-Breaking & Shadow Fix (Stabilization)**:
...
>>>>>>> feature/world-depth-expansion:.gemini/memory/current_state.md
