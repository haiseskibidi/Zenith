...
2.  **Modular Rendering Architecture**:
    - **Modular Pipeline**: Бывший монолит `Renderer.java` разделен на специализированные системы:
        ...
        - **EntityRenderSystem**: Оркестрирует отрисовку динамических сущностей и вьюмоделей первого лица. Декомпозирован на:
            - **ViewmodelRenderPass**: Настройка проекции рук, применение аффинных трансформаций освещения камеры и вызов рендерера вьюмодели.
            - **EntityRenderHelper**: Отрисовка конкретных типов сущностей (скауты, предметы, декорации, remote players, падающие блоки).
        - **OverlayRenderSystem**: Подсветка блоков, превью установки и эффекты разрушения.
    - **Chunk Mesh Generation Pipeline**: Монолит `ChunkMeshGenerator.java` декомпозирован на логически изолированные хелперы:
        - **ChunkMeshBuilder**: Высокоуровневый обход вокселей секции чанка и построение геометрии.
        - **ChunkNeighborhood**: Сбор и кэширование соседних блоков и чанков для оптимизации соседства.
        - **ChunkMeshLighting**: Расчет гладкого Ambient Occlusion и интерполяции освещения вершин.
        - **ChunkMeshWaterHelper**: Расчет высоты углов воды и сглаживания обрывов течения.
        - **ChunkMeshRampHelper**: Построение геометрии наклонных блоков (ramps).
        - **VoxelVertexHelper**: Низкоуровневые утилиты битовой упаковки вершин.
        - **MeshData**: Контейнер для хранения сырых буферов геометрии.
    ...
    - **Virtual Coordinate Origin**: 
        ...
        - **UI Transparency**: HUD (F3) автоматически вычитает оффсет, показывая игроку логические координаты, сохраняя интуитивность без усложнения кода.
    - **World Generation (Density Function AST Engine) [NEW]**:
    ...
    - **Spatial Item Merging (NEW)**: Система слияния предметов (`ItemEntity.tryMerge`) переведена на пространственное хэширование через `itemSpatialMap` в `World`. Вместо перебора всех сущностей ($O(N^2)$), предметы проверяют только соседей в своем чанке ($O(1)$), что исключает падение FPS при длительном копании.
...
7.  **Decomposed Entity Physics**: Логика движения и физики сущности вынесена из монолитного `Entity.java` в специализированные модули:
    - **EntityPhysicsProcessor**: Расчет перемещения `move()`, обработка горизонтальных/вертикальных коллизий, а также проверка коллизий на пересечение.
    - **EntityRampPhysicsHelper**: Расчет прилипания и подъема по наклонным плоскостям (рампам), пропуск препятствий над головой.
...
1. **Componentized Architecture**:
    - **PlayerViewmodelAnimator**: Оркестрирует локомоцию и анимации игрока. Управляет наложением импульсных эффектов и обновлением сглаженной высоты глаз.
    - **PlayerLocomotionState**: Хранит и рассчитывает физическое состояние движения, циклы ходьбы, наклоны, таймеры прыжков и падений с жестким соблюдением Zero-Allocation.
    - **ViewmodelBoneProcessor**: Обрабатывает кинематику скелета рук (`Shoulder -> Forearm -> Hand`), Tarkov-style коллизии рук с препятствиями и пружинную физику инерции без выделения памяти на куче.
2. **Hybrid Update Architecture**:
    - **Fixed Physical Update**: Логика приседания и физические параметры обновляются в `Player.onUpdate()`, гарантируя стабильность механик движения.
    - **Visual Render Pass (Variable FPS)**: Финальные визуальные оффсеты камеры и рук, наклоны и анимации вычисляются в `PlayerViewmodelAnimator.updateAnimations()` непосредственно перед рендерингом кадра, обеспечивая плавность при любом FPS.
3. **Impulse Overlay System**:
    - Система позволяет динамически накладывать импульсные анимации (например, `landing.json`) поверх базовой локомоции.
    - **Quadratic Velocity Scaling**: Сила визуального отклика (амплитуда `landingScale`) рассчитывается по квадратичной формуле от вертикальной скорости падения перед физическим приземлением (`Player.getPreUpdateVelocityY()`), создавая ощущение массы при прыжках с высоты.
...
5. **Spawn Settle Mechanism**:
    - Специальный фильтр первого кадра в `PlayerViewmodelAnimator`, который предотвращает "прыжки" камеры и ложные импульсы при появлении игрока в мире путем мгновенной инициализации и синхронизации физического и визуального состояний.
...
7. **Выживание**: Параметры `hunger` и `saturation` обновляются в `PlayerSurvivalComponent` (компоненте игрока). Потребление еды восстанавливает эти значения через `Player.eat(Item)` с проверкой наличия `FoodComponent`.
...
## High-Tech Graphics & Optimization
